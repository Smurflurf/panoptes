...
public class LogicalFallacyQaAgent extends AbstractAgent {
    ...
    public LogicalFallacyQaAgent(GeminiClient geminiClient, ObjectMapper mapper) {
        ...
        CRITICAL RULES:
	        ...
	        4. NO UNCITED HALLUCINATIONS: You are reading a draft that MUST be based exclusively on the cited sources. If the draft introduces specific named experiments (e.g., 'ALPHA-g'), historical events, or hard quantitative constants that are clearly NOT derived from any of the provided source abstracts, FAIL IT immediately. The writer must not rely on pre-training memory for specific facts.
	        5. If the logic is sound, units match, no fallacies are detected, and no external facts are hallucinated, valid = true.
        	6. Provide a brief, brutal reasoning. Example: "False Equivalence Error: The draft compares incommensurable metrics." or "Hallucination Error: Introduces named experiment not present in the sources."
        """);
        this.mapper = mapper;
    }
