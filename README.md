...
*   **Strict Hallucination Prevention:** A dedicated Quality Assurance Orchestrator deploys a "Panel of Auditors". It first uses Java to verify that cited IDs exist in the local database. Then, it simultaneously deploys a `CitationQaAgent` (to prevent abstract cherry-picking) and a `LogicalFallacyQaAgent` (to detect false equivalences, mismatched physical units, and biological scale conflation).
